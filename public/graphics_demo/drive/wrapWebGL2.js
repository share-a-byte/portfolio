          ...
          return function (...args) {
            ...
            if (prop == "clear") {
              ...
              for (let j = 0; j < _gl_call_traces.length; j += 1) {
                ...
                if (
                  e.length === _gl_live_call_trace.length &&
                  e.every((v, i) => v === _gl_live_call_trace[i])
                )
                  idx = j;
              }
              if (idx == -1) {
                if (_gl_call_traces.length >= 16)
                  _gl_call_traces.splice(4, 2, ["..."]); // keep compile-in-loop from creating memory leak
            ...
            if (!_gl_already_warned.has(sig)) {
              ...
              if (prop in banned) {
                console.warn("CS418 prohibits use of", prop + banned[prop]);
                _gl_already_warned.add(sig);
              }
              if (prop == "shaderSource") {
                if (!args[1].startsWith("#version 300 es")) {
                  console.warn("CS418 requires `#version 300 es` shaders");
                  _gl_already_warned.add(sig);
                }
                let src = args[1]
                  .replace(/\\(\r\n?|\n\r?)/g, "")
                ...
                if (/^[ \t\v\f]*#[ \t\v\f]*define[ \t\v\f]/m.test(src)) {
                  console.warn("CS418 prohibits use of C-style macros in GLSL");
                }
                if (args[2] == 0x8b31 && /^\s*in /m.test(src)) {
                  console.warn(
                    "CS418 requires all vertex in variables to have a layout(location=...)"
                  );
                }
                // reasonable approximation of finding GLSL inefficiencies
                let attrib = Array.from(
                  src.matchAll(
                    /\bin\b[^;]*\b([_a-zA-Z][_a-zA-Z0-9]*)\b[ \t\n\v\f\r]*;/g
                  )
                ).map((m) => m[1]);
                attrib.push(
                  "gl_VertexID",
                ...
                for (let i = 0; i < attrib.length; i += 1) {
                  [
                    ...
                    if (!attrib.includes(m[1])) attrib.push(m[1]);
                  });
                }
                if (
                  new RegExp(
                    "\\bif\\b[^{};]*\\b(" +
                      attrib.join("|") +
                      ")\\b[^{};]*\\)\\s*([{]|[a-zA-Z_])"
                  ).test(src.replace(/if\s*\([^{};]*\)\s*discard;/g, ""))
                )
                  console.warn(
                    "Shader contains `if` that may not allow full warp parallelism:"
                  );
                if (
                  new RegExp(
                    "\\bwhile\\b[^{};]*\\b(" +
                  ...
                  console.warn(
                    "Shader contains `while` that may not allow full warp parallelism"
                  );
                if (
                  new RegExp(
                    "\\bfor\\b[^{};]*;[^{};]*\\b(" +
                  ...
                  console.warn(
                    "Shader contains `for` that may not allow full warp parallelism"
                  );
              }
              if (val2 === null) {
                console.warn(`CS418 prohibits null return: ${sig}`);
                _gl_already_warned.add(sig);
              }
              if (args.some((x) => x === null)) {
                console.warn(`CS418 prohibits null arguments: ${sig}`);
                _gl_already_warned.add(sig);
              }
            }
            // log results needed for later signatures
            if (prop.startsWith("create")) {
              if (!_gl_creator_counts.has(prop))
                _gl_creator_counts.set(prop, 0);
              const thisID = _gl_creator_counts.get(prop) + 1;
              _gl_creator_counts.set(prop, thisID);
              _gl_symbols.set(val2, prop.substr(6) + "#" + thisID);
              _gl_live_call_trace[_gl_live_call_trace.length - 1] =
                prop.substr(6) +
                "#" +
                thisID +
                " = " +
...
if (window.top != window)
  window.addEventListener("message", (e) =>
    window.top.postMessage(
      [_gl_already_warned, _gl_last_call_trace, _gl_call_traces],
      "*"
    )
  );
...
requestAnimationFrame = (callback) => {
  if (_cs418_requestAnimationFrame_used != 0)
    throw new Error(
      "Calling requestAnimationFrame while another call is pending is usually an error and is prohibited in CS 418."
    );
...
cancelAnimationFrame = (requestID) => {
  if (requestID != _cs418_animationFrame_handle)
    throw new Error(
      "Calling cancelAnimationFrame without a pending animation frame request ID is usually an error and is prohibited in CS 418."
    );
  _cs418_requestAnimationFrame_used -= 1;
  _cs418_animationFrame_handle = undefined;
};
...
setTimeout = (...args) => {
  console.warn("CS 418 forbids using setTimeout");
  _cs418_setTimeout(...args);
};
setInterval = (...args) => {
  console.warn("CS 418 forbids using setInterval");
  _cs418_setInterval(...args);
};
